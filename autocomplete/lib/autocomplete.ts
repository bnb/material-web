...
export abstract class Autocomplete extends LitElement {
  ...
  protected handleClick(event: PointerEvent) {
    // When clicking the text field, open/close should toggle, however when
    // clicking other elements such as menu/items it should not.
    if (this.isOpen() &&
        this.isInComboBox(event.target as Node, this.textField!)) {
      this.close();
    } else {
      this.open();
    }
